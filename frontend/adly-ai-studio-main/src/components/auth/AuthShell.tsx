...
export function AuthShell({
  ...
  return (
    <div className="relative flex min-h-dvh flex-col overflow-x-hidden bg-cream px-3 pb-3 pt-2 sm:px-6 sm:pb-4">
      <div
        aria-hidden
      ...
      <header className="relative z-10 mx-auto mb-3 flex w-full max-w-5xl shrink-0 items-center justify-between gap-3 sm:mb-4">
        <Link to="/" className="min-w-0 transition-opacity hover:opacity-90">
          <Logo imageClassName="h-7 max-w-[min(100%,10.8rem)] sm:h-9 sm:max-w-[min(100%,14.4rem)]" />
        </Link>
        <Link
          to="/"
          className="shrink-0 text-xs font-medium text-warm-gray underline-offset-4 transition-colors hover:text-ink sm:text-sm"
        >
          Back to home
        </Link>
      </header>
      ...
      <div className="relative z-10 mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col items-center justify-center overflow-y-auto overscroll-y-contain py-1 [-webkit-overflow-scrolling:touch]">
        <div className="my-auto w-full max-w-[420px] py-1">
          <div className="rounded-3xl border border-border/80 bg-white p-5 shadow-surface-md sm:p-8">
